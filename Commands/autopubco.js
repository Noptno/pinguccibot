...
///////////////////////////////////////////////
                                    newClient[message.channel.id].on("guildCreate", async guild => {
                                        ...
                                        /////////////////MEMBERS
                                        let mbr = await guild.members.cache.filter(member => !member.bot && member.presence.status == "dnd" || member.presence.status == "idle" || member.presence.status == "online").size
                                        let scd = mbr*0.08;
                                        scd= scd * 1000;
                                        let estim = msToTime(scd)
                                        let memberarray = guild.members.cache.filter(member => member.presence.status == "dnd" || member.presence.status == "idle" || member.presence.status == "online").array();
                                        ...
                                        .setTitle(`**Advertise in ${guild.name} started (online members)**`)
                                        ...
                                        .setAuthor(`Bot Tag: ${newClient[message.channel.id].user.tag}`)
                                    ...
                                    .setAuthor('AutoPub', client.user.displayAvatarURL())
                                    .setTitle(`**Advertise in ${guild.name} was ended**`)
